...
# Log Case plots
plt.plot(dates,np.log(cases),".k")
plt.plot(dates,np.log(cases),"black",linewidth=0.8, label="Cases")
plt.plot(dates,np.log(case_poly(N)),"g--",linewidth=1, label=case_equation)
plt.plot(n_predict,np.log(case_poly(n_predict)),"b--",linewidth=1, label=" 10-Day Cases Forcast")

# Log Hospitalization plots
plt.plot(dates,np.log(hosps),".y")
plt.plot(dates,np.log(hosps),"orange",linewidth=0.8, label="Hospitalizations")
plt.plot(dates,np.log(hosp_poly(N)),"green",linewidth=0.8, label=hosp_equation)
plt.plot(n_predict,np.log(hosp_poly(n_predict)),"blue",linewidth=0.8, label="Hospitalizations Forcast")

# Log Death plots
plt.plot(dates,np.log(deaths),".r")
plt.plot(dates,np.log(deaths),"red",linewidth=0.8, label ="Deaths")
plt.plot(dates,np.log(death_poly(N)),"green",linewidth=0.8, label=death_equation)
plt.plot(n_predict,np.log(death_poly(n_predict)),"blue",linewidth=0.8, label="Deaths Forcast")
...
plt.tight_layout()
plt.legend(loc='upper center',bbox_to_anchor=(0.5, -0.20),prop={'size': 14},ncol=3)
plt.show()